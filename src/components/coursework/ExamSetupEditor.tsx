    ...
    return (
        <div className="space-y-6">
            <div className="grid gap-4 md:grid-cols-4">
                <div className="rounded-lg border bg-white p-4"><div className="text-sm text-gray-500">Questions</div><div className="mt-1 text-2xl font-bold text-gray-900">{questionCount}</div></div>
                <div className="rounded-lg border bg-white p-4"><div className="text-sm text-gray-500">Question Points</div><div className="mt-1 text-2xl font-bold text-gray-900">{totalPoints}</div></div>
                <div className="rounded-lg border bg-white p-4"><div className="text-sm text-gray-500">Attempts</div><div className="mt-1 text-2xl font-bold text-gray-900">{attempts.length}</div></div>
                <div className="rounded-lg border bg-white p-4"><div className="text-sm text-gray-500">Window</div><div className="mt-1 text-sm font-medium text-gray-900">{formatExamWindow(config.start_at, config.end_at)}</div></div>
            </div>
            ...
            <div className="rounded-lg border bg-white p-4 space-y-4">
                ...
                <div className="grid gap-4 md:grid-cols-3 lg:grid-cols-6">
                    {[["shuffle_questions", "Shuffle Questions"], ["shuffle_choices", "Shuffle Choices"], ["one_question_at_a_time", "One Question Mode"], ["no_backtracking", "No Backtracking"], ["show_results_immediately", "Show Results"], ["require_session_attendance", "Require Attendance"]].map(([field, label]) => (
                        <div key={field} className="space-y-2"><Label>{label}</Label><Select value={boolValue(config[field as keyof AssessmentExamConfig] as boolean)} onValueChange={(value) => updateConfigField(field as keyof AssessmentExamConfig, value === "yes")}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="yes">Yes</SelectItem><SelectItem value="no">No</SelectItem></SelectContent></Select></div>
                    ))}
                </div>
